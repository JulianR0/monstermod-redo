...
//=========================================================
BOOL CMBaseMonster :: FHaveSchedule( void )
{
	if ( m_pSchedule == NULL )
	{
		return FALSE;
...
// and index.
//=========================================================
void CMBaseMonster :: ClearSchedule( void )
{
...
//=========================================================
BOOL CMBaseMonster :: FScheduleDone ( void )
{
...
// to 0
//=========================================================
void CMBaseMonster :: ChangeSchedule ( Schedule_t *pNewSchedule )
{
...
#if 0
	if ( strcmp( STRING(pev->model), "models/hgrunt.mdl" ) == 0 )
	{
...
//=========================================================
void CMBaseMonster :: NextScheduledTask ( void )
{
	ASSERT( m_pSchedule != NULL );
...
//=========================================================
int CMBaseMonster :: IScheduleFlags ( void )
{
	if( !m_pSchedule )
	{
		return 0;
...
//=========================================================
BOOL CMBaseMonster :: FScheduleValid ( void )
{
	if ( m_pSchedule == NULL )
	{
...
// schedule!
//=========================================================
void CMBaseMonster :: MaintainSchedule ( void )
{
		...
		if ( !FScheduleValid() || m_MonsterState != m_IdealMonsterState )
		{
			...
			else
			{
				...
				if ( m_MonsterState == MONSTERSTATE_SCRIPT || m_MonsterState == MONSTERSTATE_DEAD )
					pNewSchedule = CMBaseMonster::GetSchedule();
				else
...
// RunTask 
//=========================================================
void CMBaseMonster :: RunTask ( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	...
	case TASK_PLAY_SEQUENCE_FACE_TARGET:
		{
			...
			if ( pTarget )
			{
				pev->ideal_yaw = UTIL_VecToYaw( pTarget->v.origin - pev->origin );
				ChangeYaw( pev->yaw_speed );
			}
...
//=========================================================
void CMBaseMonster :: SetTurnActivity ( void )
{
	float flYD;
	flYD = FlYawDiff();
...
// schedule. 
//=========================================================
void CMBaseMonster :: StartTask ( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_TURN_RIGHT:
	...
	case TASK_FIND_NEAR_NODE_COVER_FROM_ENEMY:
		{
			...
			if ( FindCover( m_hEnemy->v.origin, m_hEnemy->v.view_ofs, 0, pTask->flData ) )
			{
	...
	case TASK_FIND_FAR_NODE_COVER_FROM_ENEMY:
		{
			...
			if ( FindCover( m_hEnemy->v.origin, m_hEnemy->v.view_ofs, pTask->flData, CoverRadius() ) )
			{
				// try for cover farther than the FLData from the schedule.
				TaskComplete();
			}
	...
	case TASK_FIND_NODE_COVER_FROM_ENEMY:
		{
			...
			if ( FindCover( m_hEnemy->v.origin, m_hEnemy->v.view_ofs, 0, CoverRadius() ) )
			{
				// try for cover farther than the FLData from the schedule.
				TaskComplete();
			}
...
//				return;
	...
	case TASK_FACE_TARGET:
		if ( m_hTargetEnt != NULL )
		{
			MakeIdealYaw ( m_hTargetEnt->v.origin );
			SetTurnActivity(); 
		}
		else
			TaskFail();
	...
	case TASK_GET_PATH_TO_ENEMY:
		{
			...
			if ( BuildRoute ( pEnemy->v.origin, bits_MF_TO_ENEMY, pEnemy ) )
			{
				TaskComplete();
			}
			else if (BuildNearestRoute( pEnemy->v.origin, pEnemy->v.view_ofs, 0, (pEnemy->v.origin - pev->origin).Length() ))
			{
				TaskComplete();
			}
			else
	...
	case TASK_GET_PATH_TO_SPOT:
		{
			edict_t *pPlayer = FIND_ENTITY_BY_CLASSNAME( NULL, "player" );
...
/*jlb
				...
				if ( MoveToTarget( m_movementActivity, 2 ) )
				{
					TaskComplete();
				}
				else
...
//=========================================================
Task_t	*CMBaseMonster :: GetTask ( void ) 
{
...
// of the proper type.
//=========================================================
Schedule_t *CMBaseMonster :: GetSchedule ( void )
{
