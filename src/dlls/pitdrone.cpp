...
edict_t *CPitdroneSpike::Shoot(entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, Vector vecAngles)
{
	CPitdroneSpike *pSpit = CreateClassPtr( (CPitdroneSpike *)NULL );
...
void CMPitdrone::KeyValue(KeyValueData *pkvd)
{
...
// IgnoreConditions 
//=========================================================
int CMPitdrone::IgnoreConditions(void)
{
...
const char *CMPitdrone::pAttackMissSounds[] =
{
	"zombie/claw_miss1.wav",
	"zombie/claw_miss2.wav",
};

const char *CMPitdrone::pIdleSounds[] =
{
...
const char *CMPitdrone::pAlertSounds[] =
{
...
const char *CMPitdrone::pPainSounds[] =
{
...
const char *CMPitdrone::pDieSounds[] =
{
...
//=========================================================
// TakeDamage - overridden for pitdrone so we can keep track
// of how much time has passed since it was last injured
//=========================================================
int CMPitdrone::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	float flDist;
	Vector vecApex;
...
//=========================================================
BOOL CMPitdrone::CheckMeleeAttack1(float flDot, float flDist)
{
...
//=========================================================
BOOL CMPitdrone::CheckRangeAttack1(float flDot, float flDist)
{
	if (m_cAmmoLoaded <= 0)
	{
		return FALSE;
...
//=========================================================
void CMPitdrone::SetYawSpeed(void)
{
	int ys;
...
//=========================================================
int CMPitdrone::ISoundMask( void )
{
	return 0;
}

void CMPitdrone::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	switch (pEvent->event)
	{
	...
	case PIT_DRONE_AE_THROW:
	{
		...
		edict_t *pHurt = CheckTraceHullAttack( 70, gSkillData.pitdroneDmgWhip, DMG_SLASH );
		...
		if( pHurt )
		{
			...
			UTIL_ScreenShake( pHurt->v.origin, 15.0, 1.5, 0.7, 2 );

			pHurt->v.velocity = pHurt->v.velocity + gpGlobals->v_forward * 100;
			pHurt->v.velocity = pHurt->v.velocity + gpGlobals->v_up * 200;
		}
		else
	...
	case PIT_DRONE_AE_SLASH:
	{
		 ...
		 */
		// SOUND HERE (in the pitdrone model)
		edict_t *pHurt = CheckTraceHullAttack(70, gSkillData.pitdroneDmgBite, DMG_SLASH);
		if (pHurt)
		{
			if (pHurt->v.flags & (FL_MONSTER | FL_CLIENT))
			{
				pHurt->v.punchangle.z = shouldAttackWithLeftClaw ? 18 : -18;
				pHurt->v.punchangle.x = 5;
				pHurt->v.velocity = pHurt->v.velocity + gpGlobals->v_right * ( shouldAttackWithLeftClaw ? 100 : -100 );
			}
		}
	...
	case PIT_DRONE_AE_SPIT:
	{
		...
		if (m_hEnemy != 0)
			vecEnemyPosition = UTIL_BodyTarget(m_hEnemy, pev->origin);
		else
...
void CMPitdrone::BodyChange(float horns)
{
	if (horns <= 0)
...
void CMPitdrone::Spawn()
{
	...
	if (m_iInitialAmmo >= 0)
	{
		m_cAmmoLoaded = min(m_iInitialAmmo, PITDRONE_MAX_HORNS);
	...
	pev->classname = MAKE_STRING( "monster_pitdrone" );
	if ( strlen( STRING( m_szMonsterName ) ) == 0 )
	{
		// default name
		m_szMonsterName = MAKE_STRING( "Pit Drone" );
	}
}
...
void CMPitdrone::IdleSound(void)
{
...
// PainSound
//=========================================================
void CMPitdrone::PainSound(void)
{
...
// AlertSound
//=========================================================
void CMPitdrone::AlertSound(void)
{
...
void CMPitdrone::RunAI(void)
{
	...
	if (m_hEnemy != 0 && m_Activity == ACT_RUN)
	{
		...
		if ((pev->origin - m_hEnemy->v.origin).Length2D() < PITDRONE_SPRINT_DIST)
		{
			pev->framerate = 1.25;
		}
	}
...
void CMPitdrone::CheckAmmo( void )
{
...
void CMPitdrone::GibMonster()
{
	EMIT_SOUND( ENT( pev ), CHAN_WEAPON, "common/bodysplat.wav", 1, ATTN_NORM );

	if( CVAR_GET_FLOAT( "violence_agibs" ) != 0 )	// Should never get here, but someone might call it directly
	{
		CMGib::SpawnRandomGibs( pev, PITDRONE_GIB_COUNT, "models/pit_drone_gibs.mdl", 0 );	// Throw alien gibs
	}
	SetThink( &CMBaseEntity::SUB_Remove );
	pev->nextthink = gpGlobals->time;
}
...
DEFINE_CUSTOM_SCHEDULES(CMPitdrone)
{
...
IMPLEMENT_CUSTOM_SCHEDULES(CMPitdrone, CMBaseMonster)

//=========================================================
// GetSchedule 
//=========================================================
Schedule_t *CMPitdrone::GetSchedule(void)
{
...
// GetScheduleOfType
//=========================================================
Schedule_t* CMPitdrone::GetScheduleOfType(int Type)
{
	switch (Type)
	{
...
//=========================================================
void CMPitdrone::StartTask(Task_t *pTask)
{
...
// RunTask
//=========================================================
void CMPitdrone::RunTask(Task_t *pTask)
{
	switch( pTask->iTask )
	{
	case TASK_PDRONE_HOPTURN:
